  ...
  const Section = ({ team, players }: { team: string; players?: SquadPlayer[] }) => (
    <>
      ...
      {(players ?? []).map((p) => {
        ...
        const touched = ev && (ev.g || ev.y || ev.r);
        return (
          <tr key={p.id} className={cn("border-t border-gray-50", touched && "bg-blue-50/30")}>
            ...
            <td className="px-1 py-1.5">
              <MiniStepper tone="goal" max={maxGoals} disabled={disabled} value={ev?.g ?? 0} onChange={(v) => setField(p, team, "g", v)} />
            </td>
            <td className="px-1 py-1.5">
              <MiniStepper tone="yellow" max={1} disabled={disabled} value={ev?.y ?? 0} onChange={(v) => setField(p, team, "y", v)} />
            </td>
            <td className="px-1 py-1.5">
              <MiniStepper tone="red" max={1} disabled={disabled} value={ev?.r ?? 0} onChange={(v) => setField(p, team, "r", v)} />
            </td>
          </tr>
        );
      })}
